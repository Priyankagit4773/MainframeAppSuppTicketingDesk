  ...
  if(isset($_POST['submit'])){
      ...
      if(strlen($name) < 1) {
        ...
        try{
            ...
            $customId = date('dmY'). $savedTicket->id;

// Optional: Update the ticket record with custom ID
$sql = "UPDATE ticket SET custom_id = ? WHERE id = ?";
$stmt = $db->prepare($sql);
$stmt->bind_param("si", $customId, $savedTicket->id);
$stmt->execute();
